...
下面是一次复杂 Agent 巡检任务的精简版 2× 回放。真实 Trace 被拆成可追溯分镜，角色会在终端、档案柜、代码台、网络传送门、任务板、分身舱与 LAB 之间移动并操作对应工具。

![ReAct Agent Studio 复杂任务 2× 回放](docs/assets/react-agent-studio-2x.gif)

> GIF 为约 20 秒的循环演示。实际页面支持实时演出、导演版/精简版、暂停、逐步、拖动时间轴、变速，以及点击镜头查看原始证据。
...
短任务保留完整电影节奏；复杂任务会自动压缩停顿，导演版控制在约 90 秒内、精简版约 35 秒内，镜头和原始证据不会因此丢失。
